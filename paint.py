...
##game loop
while True:
    to_do = input("\nwould you like to paint(p), goto the store(s), sell art(e), or pay debt(d)\n")
 
    ##painting
    if to_do == "p":
        rarity1 = random.randint(1,brush_rng)
        print(f"you start painting with the {brush}\n")
    ...
    if to_do == "s":
        if brush == "dollar store brush":
            print(f"welcome to the shop, you have {money} dollars\n")
            time.sleep(2)
    ...
    ##sell paintings 
    if to_do == "e":
        print(f"you have {trash} trash paintings, {average} average paintings, {good} good paintings, {great} great paintings, and {van_go} van go paintings\n")
        time.sleep(3.5)
        if mulit_tutor == True:
    ...
    if to_do == "d":
        print(f"you have ${debt} of debt and have ${money}")
        time.sleep(2)
        paydebt = input("\nhow much of your debt would you like to pay?\n(please put in form of int)")
        if int(paydebt) <= money:
            if int(paydebt) <= debt:
                debt -= int(paydebt)
                money -= int(paydebt)
                if debt == 0:
                    print("congrats you won. gg! you are no longer in debt.\nbye bye")
                    break
